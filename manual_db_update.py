...
def create_data_path_table(cursor):
    print("Creating the data_path table...")
    cursor.execute("""
...
def create_data_vis_table(cursor):
    print("Creating the data_vis table...")
    cursor.execute("""
...
def insert_data_into_data_path_table(connection, cursor, data):
    print('Inserting data into the data_path table...')
    st_tim_d1 = time.time()
    ...
    for i in tqdm(range(0, len(data), chunk_size), desc='Executing Query', unit='query'):
...
for path in tqdm(file_paths, desc="Fetching files", unit='file'):
    ...
    if os.path.splitext(path)[0] != '.txt':
        txt_file_path = path.replace(extension_column, ".txt")
        txt_exist = 1 if os.path.exists(txt_file_path) else 0
    # print(txt_exist)

    data.append((
        None, whole_path_column, path_separator.join(components[:index + 1]), task_column, sensor_column, date_column,
        time_stamp_column, subject_name_column, ph_no_column, Unique_subjects, location_column, gender_column,
        age_column, spectacles_column, lux_values_column, traffic_data_column, run_no_column, frame_no_column,
        extension_column, txt_exist
    ))
